  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <Button icon={<GitBranch className="h-3.5 w-3.5" />} variant="secondary">Sweeps</Button>
            <Button icon={<SlidersHorizontal className="h-3.5 w-3.5" />} variant="secondary">Compare</Button>
            <Button icon={<Play className="h-3.5 w-3.5" />} variant="primary">New run</Button>
          </>
        }
      />
      ...
      <div className="space-y-6 p-8">
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          <StatCard label="Total runs" value={runs.length.toLocaleString()} sublabel={`${activeCount} live`} live={activeCount > 0} />
          <StatCard label="Finished" value={finishedCount.toString()} sublabel="last 24h" />
          <StatCard label="Failed" value={failedCount.toString()} sublabel="needs review" tone={failedCount > 0 ? "fail" : "neutral"} />
          <StatCard label="Best val_loss" value={bestVal.toFixed(3)} sublabel="↘ from 0.61" tone="success" />
        </div>
        ...
        <section className="rounded-lg border border-line bg-surface">
          <header className="flex items-center justify-between border-b border-line px-4 py-3">
            <div>
              ...
              <p className="mt-0.5 text-[12px] text-ink-3">Overlay of {Math.min(runs.length, 24)} runs · step 0 → 200,000</p>
            </div>
                  ...
                  <tr key={r.id} className="group border-b border-line/70 hover:bg-surface-2/60">
                    ...
                    <td className="py-3">
                      <Link href={`/p/${slug}/runs/${r.id}`} className="flex items-center gap-2">
                        <span className="inline-block h-2 w-2 shrink-0 rounded-sm" style={{ background: muted(i) }} />
                    ...
                    <td className="py-3 text-right font-mono tabular">{formatNum(r.config.lr as number)}</td>
...
function Button({
  children, icon, variant = "secondary",
}: { children: React.ReactNode; icon?: React.ReactNode; variant?: "primary" | "secondary" }) {
  const cls =
    variant === "primary"
  ...
  return (
    <button type="button" className={`inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-[12px] font-medium ${cls}`}>
      {icon}
      {children}
    </button>
  );
...
function StatCard({
  label, value, sublabel, live = false, tone = "neutral",
}: {
  label: string; value: string; sublabel: string;
  live?: boolean; tone?: "neutral" | "success" | "fail";
}) {
  const valueCls =
    tone === "success" ? "text-success" : tone === "fail" ? "text-fail" : "text-ink";
  return (
