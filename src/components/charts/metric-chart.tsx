...
type Props = {
  ...
  format?: "decimal" | "decimal3" | "percent" | "percent1" | "scientific" | "integer";
...
const FORMATTERS: Record<NonNullable<Props["format"]>, (v: number) => string> = {
...
const DEFAULT_GRADIENT: [string, string] = ["oklch(0.62 0.18 128)", "oklch(0.92 0.22 128)"];
...
export function MetricChart({
  ...
  const rows = useMemo(() => data.map((v, i) => ({ step: i, [dataKey]: v })), [data, dataKey]);
  ...
  return (
    <div className="flex flex-col border border-line bg-surface/40">
      ...
      <div style={{ height }} className="relative">
        ...
        >
          <Grid stroke="var(--color-line)" strokeDasharray="2 4" horizontal vertical={false} />
          <XAxis
            dataKey="step"
