...
export function Brand({ className, mono = false }: { className?: string; mono?: boolean }) {
  return (
    <span className={cn("inline-flex items-center gap-2", className)}>
      ...
      {mono ? (
        <span className="font-mono text-[12px] uppercase tracking-[0.2em] text-ink">silkworm</span>
      ) : (
        <span className="text-[16px] font-semibold tracking-tight text-ink">silkworm</span>
      )}
    </span>
  );
}
